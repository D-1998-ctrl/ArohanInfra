...
const SalesEntry = () => {
  ...
  const handleEditRow = (index) => {
    ...
    setCGST(row.CGSTPercentage || "");
    setCGSTAmount(row.CGSTAmount || "");
    setSGST(row.SGSTPercentage || "");
    setSGSTAmount(row.SGSTAmount || "");
    setIGST(row.IGSTPercentage || "");
  ...
  const GSTcal = () => {
    ...
    if (gstfromCompanyInfo === gstfromAccount) {
      // Calculate CGST and SGST if first two digits match
      cgstAmount = (amount * cgst) / 100;
      sgstAmount = (amount * sgst) / 100;
    } else {
      // Calculate IGST if first two digits don't match
...
// import React, { useMemo, useState, useEffect } from 'react'
// import { IconButton, Menu, Table, Autocomplete, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Box, useMediaQuery, Button, Typography, TextField, Drawer, Divider, FormControl, Select, MenuItem, FormControlLabel, Checkbox } from '@mui/material';
// import CloseIcon from '@mui/icons-material/Close';
// import { useTheme } from "@mui/material/styles";
// import { DatePicker } from "@mui/x-date-pickers";
// import { LocalizationProvider } from "@mui/x-date-pickers/LocalizationProvider";
...
//                 console.log(" invoicdedetailurl is used ", invoicdedetailurl)
//                 try {
//                     const response = await axios.post(invoicdedetailurl, qs.stringify(rowData), {
//                         headers: {
//                             "Content-Type": "application/x-www-form-urlencoded",
//                         },
//                     });
...
//         const row = rows[index];
//         setEditingRow(index);
//         setSelectedProduct(productOptions.find(p => p.value === row.ProductId) || null);
//         setQuantity(row.Quantity || "");
//         setRate(row.Rate || "");
//         SetAmount(row.Amount || "");
//         setCGST(row.CGSTPercentage || "");
...
//                                     onChange={(e) => setInvoiceNo(e.target.value)}
//                                     size="small" margin="none" placeholder="Invoice No" fullWidth />
//                             </Box>
...
//                                                     loading={loadingProduct}
//                                                     renderInput={(params) => (
//                                                         <TextField sx={{ minWidth: 170 }} {...params} size="small" variant="outlined" placeholder="Select Product" />
//                                                     )}
//                                                 />
//                                             </TableCell>
//                                             <TableCell>
...
//                                                 <TextField sx={{ minWidth: 120 }}  value={row.Rate || ""} onChange={handleRateChange} size="small" fullWidth />
//                                             </TableCell>
//                                             <TableCell>
